...
def calc_elem_func(func, x0, eps):
    ...
    if func == sp.sin:
        ...
        if sp.Abs(x1) <= sp.pi / 4:
            ...
            return calc_elem_func(sp.cos, sp.pi / 2 - x0, eps)
        else:
            return -calc_elem_func(sp.sin, x1 - sp.pi, eps)
...
def calc(function, x0, eps):
    ...
    if len(function.args) == 2:
        ...
        if function.func == sp.Mul:
            g, h = function.args
            bh_1, bh_2 = estimate_abs(g, x0)
            bg_1, bg_2 = estimate_abs(h, x0)
            u = calc(g, x0, eps / (bh_2 + bg_2))
            v = calc(h, x0, eps / (bh_2 + bg_2))
            return u * v
